...
class LocalListingsPage extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Scaffold(appBar: AppBar(title: Text(category.nameAr)), body: Firebase.apps.isEmpty ? const _Empty() : StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(stream: FirebaseFirestore.instance.collection(collection).where('cityId', isEqualTo: LaunchLocationDefaults.cityId).where('status', isEqualTo: category.type == CategoryType.store ? 'approved' : 'published').limit(20).snapshots(), builder: (_, snapshot) {
    if (snapshot.connectionState == ConnectionState.waiting) return const Center(child: CircularProgressIndicator());
    final docs = snapshot.data?.docs ?? const <QueryDocumentSnapshot<Map<String, dynamic>>>[];
    if (docs.isEmpty) return const _Empty();
    return ListView.separated(padding: const EdgeInsets.all(14), itemCount: docs.length, separatorBuilder: (_, __) => const SizedBox(height: 9), itemBuilder: (_, index) { final data = docs[index].data(); final title = data['name'] ?? data['title'] ?? data['employer'] ?? ''; final phone = data['phone']?.toString() ?? ''; return Card(child: ListTile(leading: const CircleAvatar(child: Icon(Icons.storefront_rounded)), title: Text(title.toString(), style: const TextStyle(fontWeight: FontWeight.w800)), subtitle: Text(data['description']?.toString() ?? ''), trailing: phone.isEmpty ? null : IconButton(onPressed: () => launchUrl(Uri.parse('tel:$phone')), icon: const Icon(Icons.phone_rounded)))); });
  }));
}
