      ...
      child: InkWell(
        ...
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Expanded(child: Text('طلب #${id.length > 8 ? id.substring(id.length - 8) : id}', style: const TextStyle(fontWeight: FontWeight.w900))),
              Text('${total.toStringAsFixed(2)} ج.م', style: const TextStyle(fontWeight: FontWeight.w900)),
            ]),
            const SizedBox(height: 9),
            Row(children: [
              Icon(_statusIcon(status), color: const Color(0xFF166534), size: 20),
              const SizedBox(width: 7),
              Text(status.labelAr, style: const TextStyle(color: Color(0xFF166534), fontWeight: FontWeight.w800)),
            ]),
        ...
        builder: (_, controller) => ListView(
          ...
          children: [
            ...
            ...items.map((item) => ListTile(
                  title: Text((item['name'] ?? 'منتج').toString()),
                  subtitle: Text('الكمية: ${item['quantity'] ?? 1}'),
                  trailing: Text('${item['price'] ?? 0} ج.م'),
                )),
            const Divider(),
            ListTile(
...
class _Timeline extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: List.generate(flow.length, (index) => Padding(
            ...
            child: Row(children: [
              ...
              Expanded(child: Text(flow[index].labelAr, style: TextStyle(fontWeight: index == currentIndex ? FontWeight.w900 : FontWeight.normal))),
            ]),
          )),
    );
  }
