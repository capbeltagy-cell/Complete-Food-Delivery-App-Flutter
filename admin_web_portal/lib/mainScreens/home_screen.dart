...
class _HomeScreenState extends State<HomeScreen> {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('إدارة ديرب', style: TextStyle(fontWeight: FontWeight.w900)), actions: [IconButton(onPressed: () => FirebaseAuth.instance.signOut(), icon: const Icon(Icons.logout_rounded), tooltip: 'تسجيل الخروج')]),
...
class _CollectionPanel extends StatelessWidget {
  ...
  Future<void> _setStatus(BuildContext context, DocumentReference<Map<String, dynamic>> ref, String status) async {
    try {
      await ref.update(<String, dynamic>{'status': status, 'updatedAt': FieldValue.serverTimestamp()});
      if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(status == 'approved' ? 'تمت الموافقة' : status == 'rejected' ? 'تم الرفض' : 'تم الإيقاف')));
    } on FirebaseException catch (e) {
      if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('تعذر تنفيذ الإجراء: ${e.code}')));
    }
  }
        ...
        builder: (context, snapshot) {
          ...
          return ListView.separated(
            ...
            itemBuilder: (_, index) {
              ...
              final data = doc.data();
              final title = data['name'] ?? data['sellerName'] ?? data['riderName'] ?? data['customerName'] ?? data['title'] ?? data['email'] ?? doc.id;
              ...
              return Card(
                child: ListTile(
                  ...
                  subtitle: Text([if (status.isNotEmpty) 'الحالة: $status', if (secondary.toString().isNotEmpty) secondary.toString()].join(' • ')),
                  trailing: section.approval
                      ? Wrap(spacing: 3, children: [
                          if (status != 'approved') IconButton(tooltip: 'موافقة', onPressed: () => _setStatus(context, doc.reference, 'approved'), icon: const Icon(Icons.check_circle_rounded, color: Colors.green)),
                          if (status != 'rejected') IconButton(tooltip: 'رفض', onPressed: () => _setStatus(context, doc.reference, 'rejected'), icon: const Icon(Icons.cancel_rounded, color: Colors.orange)),
                          if (status != 'suspended') IconButton(tooltip: 'إيقاف', onPressed: () => _setStatus(context, doc.reference, 'suspended'), icon: const Icon(Icons.block_rounded, color: Colors.red)),
                        ])
                      : null,
                ),
              );
